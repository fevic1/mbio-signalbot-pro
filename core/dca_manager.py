...
class DCAManager:
    ...
    async def open_dca_position(self, asset: str, side: str, investment_amount: float) -> Dict:
        """Canonical async entry adapter used by MCP.

        The base position is executed once. The supervisor then owns the safety
        ladder, reconciliation, adaptive repricing, and acceleration.
        """
        ...
        mids = self.executor.info.all_mids() or {}
        price = float(mids.get(asset, 0) or 0)
        if price <= 0:
            return {"success": False, "error": f"No live price available for {asset}"}

        cfg = self._global_config()
        if cfg.get("enabled") is False:
            return {"success": False, "error": "DCA is disabled by configuration"}
        allowed = {str(x).upper() for x in cfg.get("tradeable_assets", [])}
        if allowed and asset not in allowed:
            return {"success": False, "error": f"{asset} is not DCA-tradeable"}

        min_notional = float(cfg.get("min_notional", 10.0) or 10.0)
        investment = float(investment_amount)
        if investment < min_notional:
            return {"success": False, "error": f"Investment must be at least ${min_notional:.2f}"}

        base_size = investment / price
        result = await self._submit_aggressive(
            asset,
            side_to_order_side(side),
            base_size,
            "DCA_MCP_ENTRY",
        )
        if not result.get("success"):
            return {"success": False, "error": result.get("error", "Base DCA entry failed")}

        fill_price = float(result.get("avg_price", price) or price)
        levels = int(cfg.get("max_levels", cfg.get("default_levels", 3)) or 3)
        spacing = float(cfg.get("level_spacing_pct", cfg.get("default_spacing_pct", 2.0)) or 2.0)
        multiplier = float(cfg.get("size_multiplier", cfg.get("default_multiplier", 1.5)) or 1.5)
        dca = self.enable_dca(asset, levels, spacing, multiplier, side, base_size,
                              trailing=bool(cfg.get("trailing_enabled", True)),
                              trailing_offset_pct=float(cfg.get("trailing_offset_pct", 1.0)),
                              profit_target_pct=float(cfg.get("profit_target_pct", 0.0)))
        dca.update({
            "filled_levels": [1],
            "avg_entry": fill_price,
            "last_fill_price": fill_price,
            "total_invested": round(base_size * fill_price, 8),
            "last_exchange_size": base_size,
        })
        ...
        state.save_state()
        return {"success": True, "dca_id": f"DCA-{asset}-{int(time.time())}", "entry": fill_price, "size": base_size}
    ...
    def should_pause_dca(self, asset: str, current_price: float) -> bool:
        ...
        anchor = float(position.get("entry", 0) or position.get("entry_price", 0) or state.DCA_POSITIONS.get(asset, {}).get("entry_price", 0) or 0)
        if anchor <= 0:
            return False
    ...
    def _adaptive_config(self, config: Dict) -> AdaptiveDCAConfig:
        adaptive = dict(self._global_config().get("adaptive", {}) or {}); adaptive.update(config.get("adaptive", {}) or {})
        return AdaptiveDCAConfig(enabled=bool(adaptive.get("enabled", True)), aggressive_enabled=bool(adaptive.get("aggressive_enabled", True)), order_ttl_seconds=max(10.0, float(adaptive.get("order_ttl_seconds", 45.0))), min_reprice_distance_pct=max(0.01, float(adaptive.get("min_reprice_distance_pct", 0.15))), acceleration_confidence=max(0.50, min(1.0, float(adaptive.get("acceleration_confidence", 0.72)))), min_learner_score=max(0.0, min(1.0, float(adaptive.get("min_learner_score", 0.60)))), max_acceleration_multiplier=max(1.0, min(1.25, float(adaptive.get("max_acceleration_multiplier", 1.25)))), max_order_age_seconds=max(30.0, float(adaptive.get("max_order_age_seconds", 300.0))))
    ...
    async def place_dca_orders(self, asset: str, entry_price: float, base_size: float, config: Dict) -> List[Dict]:
        if self.should_pause_dca(asset, entry_price):
            logger.warning("DCA paused for %s by large-drop guardrail", asset); return []
        levels = self.calculate_dca_levels(entry_price, base_size, config); side = side_to_order_side(str(config.get("direction", "LONG"))); placed = []; now_ts = time.time()
        for level in levels:
            try:
                result = await self._submit_limit(asset, side, float(level["size"]), float(level["price"]), "DCA_ENTRY")
                if result.get("success"):
                    level.update({"order_id": result.get("order_id"), "status": "active", "placed_at": datetime.now(timezone.utc).isoformat(), "placed_at_ts": now_ts}); placed.append(level)
            except Exception as exc: logger.error("DCA order placement failed %s/L%s: %s", asset, level["level"], exc)
        config["active_orders"] = placed; config["is_waiting_for_fill"] = bool(placed); config["last_monitor_ts"] = now_ts; config["last_exchange_size"] = float(config.get("base_size", base_size) or base_size); self.active_dca[asset] = config
        if placed:
            async with state.STATE_LOCK:
                state.DCA_POSITIONS[asset] = {"entry_price": float(config.get("avg_entry", entry_price) or entry_price), "base_size": float(base_size), "direction": config.get("direction", "LONG"), "levels": placed, "is_waiting_for_fill": True, "last_fill_price": float(config.get("last_fill_price", entry_price) or entry_price), "filled_levels": list(config.get("filled_levels", [])), "opened_at": datetime.now(timezone.utc).isoformat()}; state.save_state()
        return placed

    async def _ai_advice(self, asset: str, config: Dict, market_data: dict, now_ts: float) -> dict:
        adaptive = config.setdefault("adaptive", {}); review_interval = max(15.0, float(adaptive.get("ai_review_interval_sec", 45.0)))
        if now_ts - float(adaptive.get("last_ai_review_ts", 0.0) or 0.0) < review_interval: return config.get("last_ai_advice", {})
        ...
        try:
            ...
            result, provider = await analyze_batch({asset: market_data}, get_config()); item = result.get(asset, {}) if isinstance(result, dict) else {}; signal = str(item.get("signal", "HOLD")).upper(); raw_conf = float(item.get("confidence", 0) or 0); confidence = max(0.0, min(1.0, raw_conf / 100.0 if raw_conf > 1 else raw_conf)); regime = detect_regime(market_data.get("4h", {}) or {}); learner_score = float(get_meta_learner().get_weights(regime).get("LLM", 0.0) or 0.0); direction = str(config.get("direction", "LONG")).upper(); aligned = (direction == "LONG" and signal in {"BUY", "STRONG BUY"}) or (direction == "SHORT" and signal in {"SELL", "STRONG SELL"}); advice = {"action": "ACCELERATE" if aligned else "WAIT", "confidence": confidence, "learner_score": max(0.0, min(1.0, learner_score)), "max_price": float(market_data.get("1h", {}).get("price", 0) or 0), "size_multiplier": min(1.25, float(adaptive.get("max_acceleration_multiplier", 1.25))), "valid_for_seconds": review_interval, "reason": f"{provider} {signal} ({confidence:.0%}); MetaLearner LLM weight={learner_score:.2f}"}; config["last_ai_advice"] = advice; return advice
        except Exception as exc:
            logger.warning("Adaptive DCA AI review failed for %s: %s", asset, exc); return config.get("last_ai_advice", {})
    ...
    async def _risk_budget_allows_acceleration(self, asset: str, config: Dict, proposed_size: float, price: float) -> bool:
        ...
        try:
            ...
            current_notional = sum(abs(float(p.get("size", 0) or 0)) * price for p in positions if isinstance(p, dict) and p.get("coin") == asset)
            return current_notional + abs(float(proposed_size)) * price <= balance * max_pct / 100.0 + 1e-9
        except Exception as exc: logger.error("DCA risk budget check failed for %s: %s", asset, exc); return False

    async def monitor_adaptive_dca(self, asset: str, config: Dict, current_price: float) -> Dict:
        if not config.get("enabled") or current_price <= 0: return {"action": "WAIT", "updated": 0, "cancelled": 0, "errors": []}
        adaptive = config.setdefault("adaptive", {}); now_ts = time.time(); config["last_monitor_ts"] = now_ts; self.active_dca[asset] = config
        orders = [o for o in config.get("active_orders", []) if o.get("status") == "active" and o.get("order_id")]
        if not orders: state.save_state(); return {"action": "WAIT", "updated": 0, "cancelled": 0, "errors": []}
        ttl = max(10.0, float(adaptive.get("order_ttl_seconds", 45.0)))
        stale_exists = any(now_ts - float(o.get("placed_at_ts") if o.get("placed_at_ts") is not None else now_ts) >= ttl for o in orders)
        last_price = float(adaptive.get("last_monitor_price", 0.0) or 0.0); movement_pct = abs(current_price - last_price) / last_price * 100.0 if last_price > 0 else 0.0; adaptive["last_monitor_price"] = current_price; trigger = max(0.10, float(adaptive.get("market_move_trigger_pct", 0.25)))
        if not stale_exists and movement_pct < trigger: return {"action": "WAIT", "updated": 0, "cancelled": 0, "errors": []}
        from core.data_fetcher import get_mtf_data
        try: market_data = await asyncio.to_thread(get_mtf_data, f"{asset}-USD")
        except Exception as exc: return {"action": "WAIT", "updated": 0, "cancelled": 0, "errors": [str(exc)]}
        one_hour = market_data.get("1h", {}) or {}; price = float(one_hour.get("price", current_price) or current_price); atr = float(one_hour.get("atr", 0) or 0); rsi = float(one_hour.get("rsi", 50) or 50); snapshot = MarketSnapshot(price=price, volatility=abs(atr / price) if price > 0 else 0.0, momentum=max(-1.0, min(1.0, (rsi - 50.0) / 50.0)), regime="UNKNOWN", spread_pct=0.0, liquidity_score=1.0); advice = await self._ai_advice(asset, config, market_data, now_ts); policy = self._adaptive_config(config); side = str(config.get("direction", "LONG")).upper(); results = {"action": "WAIT", "updated": 0, "cancelled": 0, "errors": []}
        for order in list(orders):
            placed_ts = order.get("placed_at_ts")
            if placed_ts is None:
                try: placed_ts = datetime.fromisoformat(str(order.get("placed_at", "")).replace("Z", "+00:00")).timestamp()
                except (TypeError, ValueError): placed_ts = now_ts
            decision = evaluate_dca_order(side=side, market=snapshot, order={**order, "placed_at_ts": placed_ts}, now_ts=now_ts, config=policy, ai_advice=advice, risk_allowed=True)
            if decision.action == DCAAction.WAIT: continue
            if decision.action == DCAAction.REPRICE:
                target = float(decision.target_price or order["price"]); cancel = await self._cancel_order(asset, int(order["order_id"]))
                if not cancel.get("success"): results["errors"].append(f"Cancel {order['order_id']}: {cancel.get('error')}"); continue
                result = await self._submit_limit(asset, side_to_order_side(side), float(order["size"]), target, "DCA_REPRICE")
                if result.get("success"): order.update({"order_id": result.get("order_id"), "price": target, "status": "active", "placed_at": datetime.now(timezone.utc).isoformat(), "placed_at_ts": now_ts}); results["action"] = "REPRICE"; results["updated"] += 1; results["cancelled"] += 1
                else: results["errors"].append(f"Reprice failed: {result.get('error', 'unknown')}")
                continue
            if decision.action == DCAAction.ACCELERATE:
                accelerated_size = float(order["size"]) * float(decision.size_multiplier)
                if not await self._risk_budget_allows_acceleration(asset, config, accelerated_size, price): results["errors"].append("Acceleration blocked by DCA exposure budget"); continue
                cancelled = 0
                for active in list(orders):
                    cancel = await self._cancel_order(asset, int(active["order_id"]))
                    if cancel.get("success"): active["status"] = "cancelled"; cancelled += 1
                if cancelled < len(orders): results["errors"].append("Acceleration aborted: not all resting DCA orders were cancelled"); continue
                result = await self._submit_aggressive(asset, side_to_order_side(side), accelerated_size, "DCA_ACCELERATED")
                if not result.get("success"): results["errors"].append(f"Acceleration failed: {result.get('error', 'unknown')}"); continue
                level = int(order.get("level", 1)); filled = config.setdefault("filled_levels", [])
                if level not in filled: filled.append(level)
                fill_price = float(result.get("avg_price", price) or price); config["last_fill_price"] = fill_price; config["last_acceleration_ts"] = now_ts; config["active_orders"] = []
                for level_data in self._build_remaining_levels(config, fill_price):
                    try:
                        ...
                        if rebuilt_result.get("success"): level_data.update({"order_id": rebuilt_result.get("order_id"), "status": "active", "placed_at": datetime.now(timezone.utc).isoformat(), "placed_at_ts": now_ts}); config["active_orders"].append(level_data)
                        else: results["errors"].append(f"Rebuild L{level_data['level']} failed: {rebuilt_result.get('error', 'unknown')}")
                    except Exception as exc: results["errors"].append(f"Rebuild L{level_data['level']} failed: {exc}")
                results.update({"action": "ACCELERATE", "updated": results["updated"] + 1, "cancelled": results["cancelled"] + cancelled}); break
        self.active_dca[asset] = config; state.save_state(); return results

    def _build_remaining_levels(self, config: Dict, anchor: float) -> List[Dict]:
        filled = {int(x) for x in config.get("filled_levels", [])}; return [x for x in self.calculate_dca_levels(anchor, float(config.get("base_size", 0) or 0), config) if int(x["level"]) not in filled]
    ...
    async def close_dca_position(self, asset: str, config: Dict, close_side: str) -> Dict:
        ...
        for order in list(config.get("active_orders", [])):
            if order.get("status") == "active" and order.get("order_id"):
                try:
                    result = await self._cancel_order(asset, int(order["order_id"])); results["dca_cancelled"] += int(bool(result.get("success")));
                    if not result.get("success"): results["errors"].append(f"Cancel {order['order_id']}: {result.get('error')}")
        ...
        config["enabled"] = False; config["active_orders"] = []; config["closed_at"] = datetime.now(timezone.utc).isoformat(); self.active_dca.pop(asset, None)
        async with state.STATE_LOCK: state.DCA_POSITIONS.pop(asset, None); state.save_state()
        return results

    async def close_dca_position_partial(self, asset: str, config: Dict, close_side: str, percent: float) -> Dict:
        percent = max(0.0, min(100.0, float(percent))); results = {"base_closed_pct": 0.0, "dca_closed": 0, "dca_cancelled": 0, "total_pnl": 0.0, "errors": []}
        for order in list(config.get("active_orders", [])):
            if order.get("status") == "active" and order.get("order_id"):
                try:
        ...
        if percent >= 100: config["enabled"] = False; config["active_orders"] = []; self.active_dca.pop(asset, None); state.DCA_POSITIONS.pop(asset, None)
    ...
    async def update_trailing_orders(self, asset: str, config: Dict, current_price: float) -> Dict:
        ...
        last_fill = float(config.get("last_fill_price", config.get("avg_entry", 0)) or 0); base_size = float(config.get("base_size", 0) or 0); side = side_to_order_side(str(config.get("direction", "LONG"))); results = {"updated": 0, "cancelled": 0, "errors": []}
        for order in list(config.get("active_orders", [])):
            if order.get("status") == "active" and order.get("order_id"):
                try:
        ...
        for level in self.calculate_trailing_levels(last_fill, current_price, base_size, config):
            ...
            if result.get("success"): level.update({"order_id": result.get("order_id"), "status": "active", "placed_at_ts": time.time()}); placed.append(level); results["updated"] += 1
    ...
    def check_profit_target(self, asset: str, config: Dict, current_price: float) -> Optional[Dict]:
        ...
        direction = str(config.get("direction", "LONG")).upper(); pnl_pct = ((current_price - avg_entry) / avg_entry * 100.0) if direction == "LONG" else ((avg_entry - current_price) / avg_entry * 100.0)
