...
The manager owns DCA order execution and adaptive policy. Runtime reconciliation
lives in ``core.dca_runtime`` so persisted state and exchange state are reconciled
before this manager is allowed to change exposure.

LLM/learner output is advisory only. Deterministic risk gates always win.
"""
...
class DCAManager:
    """Own DCA sizing, order lifecycle, adaptive decisions, and exits."""
    ...
    def enable_dca(
        ...
        adaptive.update(cfg.get("adaptive", {}) or {})
        config = {
            "enabled": True,
    ...
    def calculate_trailing_levels(self, entry_price: float, current_price: float, base_size: float, config: Dict) -> List[Dict]:
        ...
        multiplier = float(config.get("size_multiplier", config.get("multiplier", 1.25)))
        price = current_price * (1.0 - offset)
    ...
    async def _submit_limit(self, asset: str, side: str, size: float, price: float, label: str) -> dict:
        ...
        # IMPORTANT: execute_hl_order's canonical positional parameter is limit_px.
        # Passing limit_price through **kwargs silently converted DCA limits into
        # marketable defaults in the previous implementation.
    ...
    async def _submit_aggressive(self, asset: str, side: str, size: float, label: str) -> dict:
        """Submit through the verified executor's marketable-limit path."""
        from execution.hl_executor import execute_hl_order
        return await asyncio.to_thread(
            execute_hl_order,
            coin=asset,
    ...
    async def place_dca_orders(self, asset: str, entry_price: float, base_size: float, config: Dict) -> List[Dict]:
        ...
        side = "BUY" if str(config.get("direction", "LONG")).upper() == "LONG" else "SELL"
        placed = []
        now_ts = time.time()
        for level in levels:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.warning("DCA level %s failed for %s: %s", level["level"], asset, result.get("error"))
    ...
    async def _ai_advice(self, asset: str, config: Dict, market_data: dict, now_ts: float) -> dict:
        ...
        try:
            ...
            confidence_raw = float(item.get("confidence", 0) or 0)
            confidence = max(0.0, min(1.0, confidence_raw / 100.0 if confidence_raw > 1 else confidence_raw))
            regime = detect_regime(market_data.get("4h", {}) or {})
            learner_score = float(get_meta_learner().get_weights(regime).get("LLM", 0.0) or 0.0)
            direction = str(config.get("direction", "LONG")).upper()
    ...
    async def _risk_budget_allows_acceleration(self, asset: str, config: Dict, proposed_size: float, price: float) -> bool:
        ...
        try:
            ...
            return current_notional + proposed_notional <= balance * max_pct / 100.0 + 1e-9
        except Exception as exc:
    ...
    async def monitor_adaptive_dca(self, asset: str, config: Dict, current_price: float) -> Dict:
        ...
        adaptive = config.setdefault("adaptive", {})
        now_ts = time.time()
        interval = max(2.0, float(adaptive.get("monitor_interval_sec", 5.0)))
        if now_ts - float(adaptive.get("last_monitor_ts", 0.0) or 0.0) < interval:
            return {"action": "WAIT", "updated": 0, "cancelled": 0, "errors": []}
        config["last_monitor_ts"] = now_ts
        self.active_dca[asset] = config
        ...
        stale_exists = any(now_ts - float(o.get("placed_at_ts", now_ts) or now_ts) >= ttl for o in orders)
        ...
        for order in list(orders):
            ...
            decision = evaluate_dca_order(
                side=side,
                market=snapshot,
                order={**order, "placed_at_ts": placed_ts},
                now_ts=now_ts,
                config=policy,
                ai_advice=advice,
                risk_allowed=True,
            ...
            if decision.action == DCAAction.REPRICE:
                ...
                if result.get("success"):
                    order.update({"order_id": result.get("order_id"), "price": target, "status": "active", "placed_at": datetime.now(timezone.utc).isoformat(), "placed_at_ts": now_ts})
            ...
            if decision.action == DCAAction.ACCELERATE:
                ...
                filled = config.setdefault("filled_levels", [])
                if level not in filled:
                    filled.append(level)
                fill_price = float(result.get("avg_price", price) or price)
                config["last_fill_price"] = fill_price
                config["last_acceleration_ts"] = now_ts
                config["active_orders"] = []
                rebuilt = self._build_remaining_levels(config, fill_price)
                for level_data in rebuilt:
                    try:
                        ...
                        if rebuilt_result.get("success"):
                            level_data.update({"order_id": rebuilt_result.get("order_id"), "status": "active", "placed_at": datetime.now(timezone.utc).isoformat(), "placed_at_ts": now_ts})
                ...
                results.update({"action": "ACCELERATE", "updated": results["updated"] + 1, "cancelled": results["cancelled"] + cancelled})
                break
    ...
    def _build_remaining_levels(self, config: Dict, anchor: float) -> List[Dict]:
        ...
        levels = self.calculate_dca_levels(anchor, float(config.get("base_size", 0) or 0), config)
        return [level for level in levels if int(level["level"]) not in filled]
            ...
            if order.get("status") == "active" and order.get("order_id"):
                try:
                    ...
                    if result.get("success"):
                        ...
                        results["errors"].append(f"Cancel {order['order_id']}: {result.get('error')}")
                except Exception as exc:
    ...
    async def update_trailing_orders(self, asset: str, config: Dict, current_price: float) -> Dict:
        if not config.get("enabled"):
            return {"updated": 0, "cancelled": 0, "errors": []}
        adaptive = config.get("adaptive", {}) or {}
        if adaptive.get("enabled", True):
            return await self.monitor_adaptive_dca(asset, config, current_price)
        if not config.get("trailing"):
            return {"updated": 0, "cancelled": 0, "errors": []}

        last_fill = float(config.get("last_fill_price", config.get("avg_entry", 0)) or 0)
        base_size = float(config.get("base_size", 0) or 0)
        levels = self.calculate_trailing_levels(last_fill, current_price, base_size, config)
        side = side_to_order_side(str(config.get("direction", "LONG")))
        results = {"updated": 0, "cancelled": 0, "errors": []}
        for order in list(config.get("active_orders", [])):
            if order.get("status") == "active" and order.get("order_id"):
                try:
                    if (await self._cancel_order(asset, int(order["order_id"]))).get("success"):
                        results["cancelled"] += 1
                except Exception as exc:
        ...
        for level in levels:
            ...
            if result.get("success"):
                level.update({"order_id": result.get("order_id"), "status": "active", "placed_at_ts": time.time()})
                placed.append(level)
                results["updated"] += 1
        config["active_orders"] = placed
        state.save_state()
    ...
    def get_dca_status(self, asset: str) -> Optional[Dict]:
        ...
        return {
            ...
            "levels": int(config.get("levels", config.get("max_levels", 0)) or 0),
    ...
    async def synchronize(self, asset: str, position: Dict) -> Dict:
        """Compatibility entry point; runtime reconciliation owns full sync."""
