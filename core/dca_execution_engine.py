...
from core.execution_coordinator import ExecutionResult
...
class DCAExecutionEngine:
    """Own DCA order lifecycle without directly constructing an exchange client."""
    ...
    def build_ladder(
        ...
        for level in range(levels):
            ...
            price = entry * (1 - distance) if side in ("BUY", "LONG") else entry * (1 + distance)
            order_side = "BUY" if side in ("BUY", "LONG") else "SELL"
            ladder.append(
                DCAOrder(
                    level=level + 1,
                    price=round(price, 8),
                    size=self.level_size(base_size, level, multiplier),
                    side=order_side,
                )
            )
        return ladder
    ...
    async def _submit(self, asset: str, order: DCAOrder) -> ExecutionResult:
        """Use the executor's verified order path."""
    ...
    async def synchronize(self, asset: str, position: dict) -> None:
        ...
        spacing = self.spacing_pct(float(dca.get("spacing_pct", 1.2)), dca.get("spacing_pct", 1.2))

        ladder = self.build_ladder(
            entry=float(position.get("entry", position.get("entry_price", 0))),
            side=str(position.get("side", "LONG")),
            base_size=float(dca.get("base_size", position.get("size", 0))),
    ...
    async def replace_ladder(self, asset: str, ladder: List[DCAOrder], dca: dict) -> None:
        existing = list(dca.get("active_orders", []))
    ...
    async def update_trailing_orders(self, asset: str, config: dict, current_price: float) -> dict:
        """Compatibility entrypoint implemented on the current engine.

        Rebuilds the outstanding DCA ladder from the current position state after
        the strategy has marked trailing mode active. No legacy DCAManager is used.
        """
    ...
    async def check_profit_target(self, asset: str, config: dict, current_price: float) -> dict | None:
        position = state.OPEN_POSITIONS.get(asset)
        if not position or not config.get("enabled"):
            return None
...
# Backward-compatible module singleton. All exchange access is resolved through AppContext.
...
def activate_auto_dca(asset: str, direction: str, base_size: float, max_levels: int = 3,
                      spacing_pct: float = 1.2, size_multiplier: float = 1.25,
                      tp_pct: float = 2.0, sl_pct: float = 2.5, leverage: float = None) -> None:
...
async def handle_position_close_event(asset: str, pnl_usd: float, chat_id: str = "") -> None:
    ...
    if pnl_usd < 0:
        ...
        state.auto_dca_consec_losses[asset] = 0
    max_losses = 3
    if state.auto_dca_consec_losses.get(asset, 0) >= max_losses:
