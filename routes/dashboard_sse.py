...
Phase 5 DEFINITIVE: Enriched positions (margin, liq px, leverage, ROE from HL API),
analytics snapshot, safety limits, system monitoring metrics, deployed_pct guard.
This file is the single source of truth — local and container must match.
"""
import asyncio
import json
import math
...
def _compute_analytics_snapshot() -> dict:
    """Lightweight analytics for SSE. Reads trade_history.json only. Never raises."""
    snapshot = {
    ...
    snapshot["profit_factor"] = round(gross_profit / gross_loss, 2) if gross_loss > 0 else (999.99 if gross_profit > 0 else 0)
...
def _collect_system_metrics() -> dict:
    """Collect background task status, last-run timestamps, error counts. Never raises."""
    metrics = {
        "tasks": {},
        "uptime_sec": int(time.time() - _start_time) if '_start_time' in dir() else 0,
        "auto_trading": os.environ.get("ENABLE_AUTO_TRADING", "false").lower() == "true",
        "last_update": datetime.now(timezone.utc).isoformat(),
    }

    # Known background tasks from main.py registration
    known_tasks = [
    ...
    try:
        ...
        for task_name in known_tasks:
            ...
            if last_run:
                try:
                    ...
                    if age_sec > 600:  # Stale if >10 min without check-in
                        status = "stale"
                except Exception:
                    status = "unknown"
            else:
        ...
        logger.debug(f"System metrics collection: {e}")
        for task_name in known_tasks:
            metrics["tasks"][task_name] = {"status": "unknown", "last_run": "", "error_count": 0}
...
async def _collect_dashboard_state() -> dict:
    """Collect live state from core modules. Never raises."""
    state_data = {
    ...
    try:
        ...
        try:
            ...
            state_data["balance"] = round(balance, 2)
        except Exception:
            pass

        # Positions with live enrichment from HL API user_state()
        try:
            ...
            try:
                ...
                logger.debug(f"SSE exchange position fetch: {e}")
            ...
            for key, pos in state.OPEN_POSITIONS.items():
                ...
                if side == "BUY":
                    upnl = (current - entry) * size if current > 0 else 0
                else:
                    upnl = (entry - current) * size if current > 0 else 0

                pnl_pct = (((current - entry) / entry * 100) if side == "BUY"
                ...
                state_data["positions"].append({
                    ...
                    "size": round(size, 8), "entry": round(entry, 4),
                    "current": round(current, 4), "upnl": round(upnl, 4),
                    ...
                    "leverage": round(exch.get("leverage", 1), 1),
                    "roe": round(exch.get("roe", 0) * 100, 2),
                    "sl": round(float(pos.get("sl", 0)), 4),
                    "tp1": round(float(pos.get("tp1", 0)), 4),
                    "tp2": round(float(pos.get("tp2", 0)), 4),
                    "tp3": round(float(pos.get("tp3", 0)), 4),
            ...
            total_notional = sum(
                p["size"] * p["current"] for p in state_data["positions"] if p["current"] > 0
            )
            if state_data["balance"] > 0:
                state_data["deployed_pct"] = round(total_notional / state_data["balance"] * 100, 1)
            else:
                state_data["deployed_pct"] = 0.0

        except Exception as e:
            logger.debug(f"SSE position fetch: {e}")
        ...
        try:
            ...
            logger.debug(f"SSE grid fetch: {e}")

        # Analytics snapshot
        try:
            state_data["analytics"] = _compute_analytics_snapshot()
        except Exception as e:
            logger.debug(f"SSE analytics: {e}")
        ...
        # System monitoring metrics (Phase 5)
        try:
            state_data["system"] = _collect_system_metrics()
        except Exception as e:
            logger.debug(f"SSE system metrics: {e}")

    except Exception as e:
        logger.error(f"SSE state collection error: {e}")

    return state_data


async def sse_generator():
    """Yield SSE-formatted JSON events every 2 seconds."""
    while True:
        try:
...
async def dashboard_sse_stream():
    """Return StreamingResponse for SSE endpoint."""
