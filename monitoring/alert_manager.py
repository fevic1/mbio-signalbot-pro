...
def _safe_md(text: str) -> str:
    """Escape Telegram MarkdownV2 special chars. Fallback to plain text if needed."""
    if not isinstance(text, str):
        text = str(text)
    # Escape reserved chars: _ * [ ] ( ) ~ ` > # + - = | { } . !
    escaped = text
    for char in ['_', '*', '[', ']', '(', ')', '~', '`', '>', '#', '+', '-', '=', '|', '{', '}', '.', '!']:
        escaped = escaped.replace(char, f'\\{char}')
    return escaped

async def _send_safe(bot, chat_id, text, parse_mode='Markdown'):
    """Send message with automatic fallback to plain text on error."""
    try:
        await bot.send_message(chat_id=chat_id, text=_safe_md(text), parse_mode=parse_mode)
        ...
        try:
            ...
            logger.error(f"Telegram send failed completely: {e}")
...
import os; os.environ.setdefault("CHROMA_TELEMETRY_DISABLED", "true")
...
async def safe_send_message(chat_id, text, parse_mode=None):
    ...
    try:
        ...
        # Use the global application instance
        import monitoring.alert_manager as self_mod
        if hasattr(self_mod, '_application') and self_mod._application:
            await self_mod._application.bot.send_message(
...
def _escape_md(text: str) -> str:
    """Escape Telegram MarkdownV2 special characters to prevent parse errors."""
    if not isinstance(text, str):
        text = str(text)
...
async def _safe_send_telegram(chat_id, text: str, parse_mode='Markdown'):
    """Send Telegram message with automatic fallback to plain text on parse failure."""
    try:
        if TELEGRAM_BOT and chat_id:
            await _send_safe(TELEGRAM_BOT, chat_id, text=text, parse_mode=parse_mode)
    except Exception as e:
        # Fallback: strip all markdown and resend as plain text
        import re
        plain = re.sub(r'[*_`~\[\](){}#+\-=|.!>]', '', str(text))
        try:
            if TELEGRAM_BOT and chat_id:
                await _send_safe(TELEGRAM_BOT, chat_id, text=plain)
        except Exception:
            logger.warning(f"⚠️ Telegram send failed completely: {e}")


async def _safe_telegram(chat_id, text: str, parse_mode='Markdown'):
    """Send with automatic plain-text fallback on parse failure."""
    try:
        if TELEGRAM_BOT and chat_id:
            await _send_safe(TELEGRAM_BOT, chat_id, text=text, parse_mode=parse_mode)
    except Exception:
        ...
        try:
            if TELEGRAM_BOT and chat_id:
                await _send_safe(TELEGRAM_BOT, chat_id, text=plain[:4000])
        except Exception as e2:
            logger.warning(f"Telegram send failed: {e2}")
...
async def cmd_positions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for asset, pos in state.OPEN_POSITIONS.items():
        ...
        msg += f"📦 *Size:* `{size:.4f}`\n"
        msg += f"💎 *Value:* `${size * entry:,.2f}`\n\n"
        
        msg += f"📈 *PERFORMANCE & RISK*\n"
        msg += f"├─ {pnl_emoji} *uPnL:* `{pnl_sign}${pnl:,.2f}` ({pnl_sign}{pnl_pct:.2f}%)\n"
        msg += f"├─ 🛡 *SL:* `${sl:,.4f}`\n"
        msg += f"├─ 🏁 *TP1:* `${tp1:,.4f}`\n"
        msg += f"├─ 🏁 *TP2:* `${tp2:,.4f}`\n"
        msg += f"└─ 🏁 *TP3:* `${tp3:,.4f}`\n"
...
async def cmd_close(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Close a position (full or partial) including all DCA levels. Per DCA.md Step 3 + DCA(1).md Feature 2."""
    if not update.message or not update.message.text:
        return
    parts = update.message.text.split()
    if len(parts) < 2:
        await update.message.reply_text("Usage: /close <ASSET> [PERCENT]\nExamples:\n/close BTC (full)\n/close BTC 50% (half)")
        return
    ...
    try:
        ...
        if dca_config and dca_config.get("enabled"):
            ...
            if percent >= 100:
                ...
                msg += f"├ Total PnL: ${result['total_pnl']:+.4f}\n"
                if result["errors"]:
                    msg += f"└ Errors: {', '.join(result['errors'][:3])}"
                else:
                    msg += "└ Status: All closed successfully"
            else:
                result = await dca.close_dca_position_partial(asset, dca_config, close_side, percent)
                msg = f"✅ Closed {percent}% of {asset} DCA position\n"
                msg += f"├ DCA orders cancelled: {result['dca_cancelled']}\n"
                msg += f"├ Total PnL: ${result['total_pnl']:+.4f}\n"
                if result["errors"]:
                    msg += f"└ Errors: {', '.join(result['errors'][:3])}"
                else:
                    msg += "└ Status: Partial close successful"
            await update.message.reply_text(msg)
        else:
            # Standard close (no DCA)
            from execution.hl_executor import execute_hl_order
            result = await asyncio.to_thread(
                execute_hl_order, coin=asset, side=close_side,
            ...
            if result.get("success"):
                ...
                logger.info(f"💰 {asset} PnL: ${pnl_usd:+.2f}")
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        msg = (
            ...
            f"└─ 🔒 Deployed: *{deployed_pct:.0f}%* {'🟢' if deployed_pct <= 500 else '🟡' if deployed_pct <= 700 else '🔴'}\n"
            f"      `{'█' * min(10, int(deployed_pct/100))}{'░' * max(0, 10-int(deployed_pct/100))}`\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📈 *PERFORMANCE METRICS*\n"
            f"├─ {'🟢' if daily_pnl > 0 else '🔴' if daily_pnl < 0 else '🟡'} Daily PnL: *{daily_pnl:+.2f}%*\n"
            f"├─ 💰 Realized: *${realized_pnl_usd:+.2f}* (closed trades)\n"
            f"├─ 📊 Unrealized: *${unrealized_pnl_usd:+.2f}* (open positions)\n"
            f"├─ 💹 Total PnL: *${total_pnl_usd:+.2f}*\n"
            f"├─ 🎯 Win Rate: *{win_rate_str}*\n"
            f"└─ 🏆 Top Strategy: *{top_strat}*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⚙️ *SYSTEM STATUS*\n"
            f"├─ 🔄 Position Monitor: *60s* ✅\n"
            f"├─ 🔍 Quick Scanner: *{quick_min}min* ✅\n"
            f"├─ 🎯 Entry Scanner: *{entry_min}min* ✅\n"
            f"├─ 🔄 Full Analysis: *{full_hours}h*\n"
            f"└─ 🧠 AI Providers: *{providers}*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🛡️ _Risk Limit: 500% max for NEW trades_\n"
            f"`{now_str}`"
        )
...
async def cmd_dca_chart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show DCA levels as ASCII chart in Telegram. Per DCA(1).md Feature 4."""
    if not update.message or not update.message.text:
        return
    parts = update.message.text.split()
    if len(parts) < 2:
    ...
    try:
        ...
        # Get current price via verified HLExecutor API
        ...
        if not levels:
            ...
            for level in levels_sorted:
                ...
                status_icon = "✅" if status == "filled" else "⏳" if status == "active" else "❌"
                lines.append(f"L{level_num}: ${price:,.2f} {arrow} {abs(distance_pct):.2f}% | {size:.6f} {status_icon}")
        ...
        if target_pct:
            ...
            status = "🎯 HIT" if pnl_pct >= target_pct else f"⏳ {pnl_pct:.1f}%/{target_pct}%"
            lines.append(f"Profit target: {status}")
...
async def cmd_open_grid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Open a GRID bot for sideways trading. Isolated from DCA via GRID:: namespace."""
    if not update.message or not update.message.text:
        return
    parts = update.message.text.split()
    if len(parts) < 7:
        await update.message.reply_text(
            "🔲 *GRID Bot Setup*\n\n"
            "Usage: `/open_grid <ASSET> <LOWER> <UPPER> <LEVELS> <INVESTMENT> <PROFIT_PCT>`\n\n"
            "Example:\n`/open_grid BTC 59000 61000 10 100 0.5`\n"
            "• Range: $59,000-$61,000\n• 10 levels\n• $100 USDC\n• 0.5% profit/grid",
            parse_mode='Markdown'
        )
        return
    ...
    try:
        ...
        # 🛡️ Minimum investment safeguard for exchange order minimums
        min_investment = 20  # Minimum USDC to open a viable grid
        if investment < min_investment:
            await update.message.reply_text(
                f"❌ Investment too low. Minimum ${min_investment} USDC required to meet exchange order minimums.\n\n"
                f"Try: `/open_grid BTC 59000 61000 5 {min_investment} 0.5`"
            )
            return
        ...
        _grid_cfg = _cfg.get_config().get("grid", {}).get("reversal", {})
        ...
        # Store in namespaced state key — NO collision with DCA
        state.OPEN_POSITIONS[grid_state_key(asset)] = config
        state.save_state()

        msg = (
            f"✅ *GRID Bot Activated: {asset}*\n\n"
            f"📊 Range: ${lower_price:,.2f}-${upper_price:,.2f}\n"
            f"🔢 Levels: {grid_quantity} (spacing: ${config.get('step_size', config.get('grid_spacing', 0)):.2f})\n"
        ...
        # Record grid activation in trade ledger
        ...
        # Persist grid state to survive restarts
        ...
        logger.error(f"❌ Error in cmd_open_grid: {e}")
        await update.message.reply_text(f"💥 Error: {str(e)}")
...
async def cmd_close_grid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    parts = update.message.text.split()
    if len(parts) < 2:
        await update.message.reply_text("Usage: `/close_grid <ASSET>`")
        return
    ...
    try:
        ...
        if result["errors"]:
            msg += f"\n⚠️ Errors: {', '.join(result['errors'][:3])}"
        # Record grid closure in trade ledger
        ...
        # Clear persisted grid state on close
...
async def send_grid_alert(message: str) -> None:
    ...
    try:
        ...
        if chat_id and TELEGRAM_BOT:
            await _safe_telegram(chat_id, text=message)
    except Exception as e:
...
async def grid_monitor_task():
    ...
    Runs every 2 minutes. Detects fills and triggers reversal flips automatically."""
    import asyncio
    import core.state as state
        ...
        try:
            ...
            for key in list(state.OPEN_POSITIONS.keys()):
                ...
                if fills > 0:
                    ...
                    # Send periodic status update every ~30 min (15 cycles at 2-min interval)
                ...
                # Save updated config back to state
                state.OPEN_POSITIONS[key] = config
...
async def cmd_trade_history(update, context):
    """Display trade history using HTML parse mode (immune to MarkdownV2 failures)."""
    chat_id = update.effective_chat.id
    try:
        ...
        # Show last 10 trades in HTML format (no MarkdownV2 parsing issues)
        ...
        for trade in history[-10:]:
            ...
            strategy = trade.get("strategy", "SIGNAL")
            
            emoji = "🟢" if pnl >= 0 else "🔴"
            lines.append(
                f"{emoji} <b>{asset}</b> {side} {size}\n"
                f"   💲 Price: ${price:.4f} | PnL: ${pnl:+.4f}\n"
                f"   🏷️ {strategy} | ⏰ {ts}"
            )
            lines.append("────────────────────")
        ...
        # Send as HTML — immune to underscore/dash/period parsing failures
        await context.bot.send_message(
            chat_id=chat_id,
            text=message,
            parse_mode='HTML'
        ...
        # Nuclear fallback: send as plain text with zero formatting
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"Trade history available ({len(load_history())} records). Display error: {str(e)[:100]}"
            )
        except Exception:
            pass
