...
async def check_and_close_positions(chat_id: str) -> None:
    ...
    for _asset, _pos in list(state.OPEN_POSITIONS.items()):
        ...
        _opened_at = _pos.get("opened_at")

        if isinstance(_opened_at, str):
            try:
                _opened_at = datetime.fromisoformat(
                    _opened_at.replace("Z", "+00:00")
                )
    ...
    for asset, pos in list(state.OPEN_POSITIONS.items()):
        ...
        try:
            ...
            if not isinstance(cfg_age, dict):
                cfg_age = {}
            ...
            if side == "BUY":
                if current_price >= pos.get("tp3", float("inf")):
                    ...
                    # Executable TP1:
                    # A partial close is valid only when both the
                    # closed portion and the remaining position satisfy
                    # the exchange minimum notional.
                    requested_pct = float(
                        config.get("smart_exit", {}).get(
                            "tp2_partial_pct",
                            0.35,
                        )
                    )
                    ...
                    min_notional = float(
                        get_exchange_limits()["min_notional_usd"]
                    )
                    current_notional = size * current_price
                    requested_close_size = size * requested_pct
                    requested_close_notional = (
                        requested_close_size * current_price
                    )

                    executable_partial = (
                        requested_close_size > 0
                        and requested_close_notional >= min_notional
                        and (size - requested_close_size) * current_price >= min_notional
                    )
                    ...
                    if executable_partial:
                        ...
                        try:
                            res = execute_hl_order(
                                ...
                                close_size,
                                reduce_only=True,
                                execution_label="SIGNAL_EXIT",
                            )

                            if res.get("success"):
                                closed_pct = (close_size / size) * 100.0
                                ...
                                pos["sl"] = entry * 1.002
                                pos["tp1_hit"] = True
                                pos["tp1_partial_skipped"] = False

                                await send_tp_hit(
                                    asset,
                                    f"TP1 ({closed_pct:.1f}% Closed)",
                                    current_price,
                                    entry,
                                    chat_id,
                                )
                            ...
                            logger.error(
                                f"❌ Partial TP1 failed: {e}"
                            )
                            pos["tp1_hit"] = False
                            pos["tp1_partial_skipped"] = False

                    else:
                        # The position is too small for an executable
                        # partial close. Record TP1 as a milestone only.
                        pos["tp1_hit"] = True
                        pos["tp1_partial_skipped"] = True
                        pos["tp1_execution"] = "MILESTONE_ONLY"
                        pos["sl"] = entry * 1.002

                        logger.info(
                            f"📌 {asset} TP1 milestone reached; "
                            f"partial skipped: "
                            f"position=${current_notional:.2f}, "
                            f"minimum=${min_notional:.2f}, "
                            f"requested=${requested_close_notional:.2f}"
                        )

                        await send_tp_hit(
                            asset,
                            "TP1 (milestone - position too small)",
                            current_price,
                            entry,
                            chat_id,
                        )
                ...
                if current_price <= pos.get("tp3", 0):
                    ...
                    # Executable TP1:
                    # Only submit a partial close when both the close and
                    # remaining position can satisfy the exchange minimum.
                    requested_pct = float(
                        config.get("smart_exit", {}).get(
                            "tp2_partial_pct",
                            0.35,
                    ...
                    min_order_size = min_notional / current_price
                    ...
                    if executable_partial:
                        ...
                        # The position is too small for an executable partial.
                        # Do not pretend a profit was taken.
                        pos["tp1_hit"] = True
                        pos["tp1_partial_skipped"] = True
                        pos["tp1_execution"] = "MILESTONE_ONLY"
                        pos["sl"] = entry * 0.998
            ...
            if should_close:
                ...
                if not result.get("success"):
                    logger.error(
                        f"❌ Failed to close {asset}: "
                        f"{result.get('error', 'unknown')}"
                    )
                    continue

                # Exchange is authoritative for completion of a normal
                # signal trade. Do not record the trade locally until the
                # exchange confirms the position is flat.
                exchange_flat = False

                for _verify_attempt in range(8):
                    try:
                        await asyncio.sleep(1.0)

                        live_positions = await run_executor_method(
                            app_context.executor.get_open_positions
                        )
                        ...
                        live_position = next(
                            (
                                ...
                                if p.get("coin") == hl_assets.get(asset, asset)
                            ),
                            None,
                        )
                        ...
                        logger.warning(
                            "⚠️ %s exchange close verification "
                            "attempt %d failed: %s",
                            asset,
                            _verify_attempt + 1,
                            verify_exc,
                ...
                if not exchange_flat:
                    logger.error(
                        f"❌ {asset} close not exchange-confirmed after "
                        f"{8}s; retaining local position state."
                    )
                    continue

                # Canonical realized exit price comes from the actual
                # Hyperliquid fill, not the monitor's sampled market price.
                exit_price = float(result.get("avg_price") or 0)

                if exit_price <= 0:
                    logger.error(
                        f"❌ {asset} close confirmed but exchange returned "
                        f"no valid avg_price; retaining local position state."
                    )
                    continue
