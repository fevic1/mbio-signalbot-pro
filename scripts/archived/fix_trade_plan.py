...
old_call = '_tp_s = calculate_trade_plan(get_account_balance(), data["1h"]["price"], data["1h"]["atr"])'
new_call = '_tp_s = calculate_trade_plan(get_account_balance(), data["1h"]["price"], data["1h"]["atr"], signal)'

if old_call in content:
    content = content.replace(old_call, new_call)
    print("✅ Fixed calculate_trade_plan - added signal parameter")

# Also fix line 234 if needed
